...
- `web/firestore.rules` → список в `isMember()` — собственно запрет.

Данные у каждого свои. Владелец (`FINANCE_OWNER_EMAIL` в `web/config.js` и почта
в `isOwner()` правил) работает с корневыми коллекциями, каждый другой допущенный —
только со своим пространством `spaces/<почта>`: при первом входе у него пустая база
со стандартными статьями. Друг друга они не видят. После правки — `npm run deploy`.
